-- ====================================
-- BUSINESS TABLES MIGRATION FOR JURY TESTING
-- Created for DataBoard Demo System
-- ====================================
...
CREATE TABLE IF NOT EXISTS orders (
    ...
    estimated_delivery_time INTEGER, -- hours
...
-- ====================================
-- 8. VALIDATION AND AUDIT LOG TABLE
-- ====================================
...
CREATE INDEX IF NOT EXISTS idx_audit_date ON data_audit_log(created_at);
CREATE INDEX IF NOT EXISTS idx_audit_user ON data_audit_log(user_id);

-- ====================================
-- 9. DEDUPLICATION TRACKING
-- ====================================
CREATE TABLE IF NOT EXISTS deduplication_log (
    ...
    resolution_action VARCHAR(50), -- 'merge', 'keep_original', 'keep_duplicate', 'manual_review'
...
('CUST003', 'О��О "Офис Центр"', 'office@office-center.ru', '+7-495-345-6789', 'ООО "Офис Центр"', '{"street": "ул. Деловая 45", "city": "Москва", "postal_code": "115432", "country": "Россия"}', 'active', 200000.00, '2024-01-10'),
...
('PROD002', 'Смартфон Samsung Galaxy A54', 'Смартфон с экраном 6.4", 128GB, камера 50MP', 3, 1, '8806094559589', 'SAM-A54-128-BLK', 28000.00, 39900.00, 26500.00, 42.52, 25, 10, 'B-2-08', '{"display": "6.4 Super AMOLED", "storage": "128GB", "camera": "50MP + 12MP + 5MP", "battery": "5000mAh", "os": "Android 13"}', 'Samsung', ARRAY['смартфон', '��ндроид', 'камера', 'samsung']),
...
('PROD008', 'Наушники Sony WH-1000XM5', 'Беспроводные наушники с шумоподавлением', 1, 3, '4548736141853', 'SONY-WH1000XM5-BLK', 22000.00, 32900.00, 21200.00, 49.54, 18, 6, 'C-2-18', '{"type": "Накладные", "connection": "Bluetooth 5.2", "anc": "Активное шумо��одавление", "battery": "30 часов", "quick_charge": "3 мин = 3 часа"}', 'Sony', ARRAY['наушники', 'беспроводные', 'шумоподавление', 'sony'])
...
(5, 'out', 2, 15, 13, 'order', 2, 'Продажа по заказу ORD-2024-002', 'system')
...
-- Update supplier totals based on products sold
...
SELECT 
    ...
    'Клиенты и их контактная информация' as description
FROM customers
UNION ALL
SELECT 
    'Suppliers' as table_name, 
    COUNT(*) as record_count,
    'Поставщики товаров' as description
FROM suppliers
UNION ALL
SELECT 
    'Products' as table_name, 
    COUNT(*) as record_count,
    'Товары и ��слуги' as description
FROM products
UNION ALL
SELECT 
...
UNION ALL
SELECT 
    'Inventory Movements' as table_name, 
    COUNT(*) as record_count,
